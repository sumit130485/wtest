    ...
    class OVIC_Field_spacing extends OVIC_Fields
    {
        ...
        public function render()
        {
            ...
            if (!empty($args['all'])) {
                ...
                echo '<input type="number" name="'.esc_attr($this->field_name('[all]')).'" value="'.esc_attr($value['all']).'"'.$placeholder.' class="ovic-input-number'.esc_attr($is_unit).'" step="any" />';
                echo ($unit) ? '<span class="ovic--label ovic--unit">'.esc_attr($args['units'][0]).'</span>' : '';
                echo '</div>';
                ...
                foreach ($properties as $property) {
                    ...
                    echo '<input type="number" name="'.esc_attr($this->field_name('['.$property.']')).'" value="'.esc_attr($value[$property]).'"'.$placeholder.' class="ovic-input-number'.esc_attr($is_unit).'" step="any" />';
                    echo ($unit) ? '<span class="ovic--label ovic--unit">'.esc_attr($args['units'][0]).'</span>' : '';
                    echo '</div>';
