...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Ovic_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @param  array  $args  Arguments to pass to Ovic_Breadcrumb_Trail.
 *
 * @return string
 * @since  0.1.0
 * @access public
 */
function ovic_breadcrumb_trail($args = array())
{
    $breadcrumb = apply_filters('ovic_breadcrumb_trail_object', null, $args);

    if (!is_object($breadcrumb)) {
        $breadcrumb = new Ovic_Breadcrumb_Trail($args);
    }
 ...
 * @access public
 */
if (!class_exists('Ovic_Breadcrumb_Trail')) {
    class Ovic_Breadcrumb_Trail
    {
        ...
        /**
         * Sets up the breadcrumb trail properties.  Calls the `Ovic_Breadcrumb_Trail::add_items()` method
         * to creat the array of breadcrumb items.
         *
         * @param  array  $args  {
         *
         * @type string $container Container HTML element. nav|div
         * @type string $delimiter The character to display between the breadcrumbs.
         * @type string $wrap_before The breadcrumb’s container starting code
         * @type string $wrap_after The breadcrumb’s container ending code
         * @type string $before HTML to display before the breadcrumbs.
         * @type string $after HTML to display after the breadcrumbs.
         ...
         * @type array $labels Text labels. @see Ovic_Breadcrumb_Trail::set_labels()
         * @type array $post_taxonomy Taxonomies to use for post types. @see Ovic_Breadcrumb_Trail::set_post_taxonomy()
        ...
        public function __construct($args = array())
        {
            ...
            $this->args = apply_filters('ovic_breadcrumb_trail_args', wp_parse_args($args, $defaults));
        ...
        public function trail()
        {
            ...
            if (0 < $item_count) {
                ...
                $breadcrumb .= sprintf('<%s class="trail-items breadcrumb"%s>', tag_escape($this->args['list_tag']),
                    true === $this->args['meta'] ? ' itemscope itemtype="http://schema.org/BreadcrumbList"' : ''
                );
                ...
                foreach ($this->items as $key => $item) {
                    ...
                    $item = !empty($matches) ? sprintf('%s<span%s>%s</span>%s', $matches[1], true === $this->args['meta'] ? ' itemprop="name"' : '', $matches[2], $matches[3]) : sprintf('<span%s>%s</span>', true === $this->args['meta'] ? ' itemprop="name"' : '', $item);

                    // Wrap the item with its itemprop.
                    if (!empty($matches) && true === $this->args['meta']) {
                    ...
                    if (true === $this->args['meta']) {
                        $meta       = sprintf('<meta itemprop="position" content="%s" />', absint($item_position));
                        $attributes .= ' itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"';
                    }

                    // Build wrap item HTML.
                    $item = sprintf('%1$s%2$s%3$s', $this->args['before'], $item, $this->args['after']);

                    // Build wrap delimiter item HTML.
                    if ($key > 0 && !empty($this->args['delimiter'])) {
                        $item = sprintf('%1$s%2$s', $this->args['delimiter'], $item);
                    }
                ...
                $breadcrumb = sprintf(
                    '%3$s<%1$s role="navigation" aria-label="%2$s" class="breadcrumb-trail breadcrumbs">%4$s</%1$s>%5$s',
                    tag_escape($this->args['container']),
                    esc_attr($this->labels['aria_label']),
                    $this->args['wrap_before'],
                    $breadcrumb,
                    $this->args['wrap_after']
                );
            }

            // Allow developers to filter the breadcrumb trail HTML.
            $breadcrumb = apply_filters('ovic_breadcrumb_trail', $breadcrumb, $this->args);
        ...
        protected function set_labels()
        {
            ...
            $defaults = array(
                'browse'              => esc_html__('Browse:', 'ovic-addon-toolkit'),
                'aria_label'          => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'ovic-addon-toolkit'),
                'home'                => esc_html__('Home', 'ovic-addon-toolkit'),
                'error_404'           => esc_html__('404 Not Found', 'ovic-addon-toolkit'),
                'archives'            => esc_html__('Archives', 'ovic-addon-toolkit'),
                // Translators: %s is the search query.
                'search'              => esc_html__('Search results for: %s', 'ovic-addon-toolkit'),
                // Translators: %s is the page number.
                'paged'               => esc_html__('Page %s', 'ovic-addon-toolkit'),
                // Translators: %s is the page number.
                'paged_comments'      => esc_html__('Comment Page %s', 'ovic-addon-toolkit'),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute'      => esc_html__('Minute %s', 'ovic-addon-toolkit'),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        => esc_html__('Week %s', 'ovic-addon-toolkit'),
            ...
            $this->labels = apply_filters('ovic_breadcrumb_trail_labels', wp_parse_args($this->args['labels'], $defaults));
        }
        ...
        protected function set_post_taxonomy()
        {
            ...
            $this->post_taxonomy = apply_filters('ovic_breadcrumb_trail_post_taxonomy', wp_parse_args($this->args['post_taxonomy'], $defaults));
        }
        ...
        protected function add_items()
        {
            ...
            $this->items = array_unique(apply_filters('ovic_breadcrumb_trail_items', $this->items, $this->args));
        }
        ...
        protected function add_minute_hour_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_minute_hour'], get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'ovic-addon-toolkit')));
            }
        }
        ...
        protected function add_minute_archive_items()
        {
            ...
            // Add the minute item.
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_minute'], get_the_time(esc_html_x('i', 'minute archives time format', 'ovic-addon-toolkit')));
            }
        }
        ...
        protected function add_hour_archive_items()
        {
            ...
            // Add the hour item.
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_hour'], get_the_time(esc_html_x('g a', 'hour archives time format', 'ovic-addon-toolkit')));
            }
        }
        ...
        protected function add_day_archive_items()
        {
            ...
            $year  = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'ovic-addon-toolkit')));
            $month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'ovic-addon-toolkit')));
            $day   = sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'ovic-addon-toolkit')));
        ...
        protected function add_week_archive_items()
        {
            ...
            $year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'ovic-addon-toolkit')));
            $week = sprintf($this->labels['archive_week'], get_the_time(esc_html_x('W', 'weekly archives date format', 'ovic-addon-toolkit')));
         ...
         * @access protected
         */
        protected function map_rewrite_tags(int $post_id, $path)
        {
            ...
            if (is_array($matches)) {
                ...
                foreach ($matches as $match) {
                    ...
                    if ('%year%' == $tag) {
                        $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'ovic-addon-toolkit'))));
                    } // If using the %monthnum% tag, add a link to the monthly archive.
                    elseif ('%monthnum%' == $tag) {
                        $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'ovic-addon-toolkit'))));
                    } // If using the %day% tag, add a link to the daily archive.
                    elseif ('%day%' == $tag) {
                        $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'ovic-addon-toolkit'))));
