...
if ( !class_exists( 'MCAPI' ) ) {
	class MCAPI
	{
		...
		private function show_error( $message )
		{
			...
			add_settings_error( 'ovic-api', 'ovic-api-error', $message, 'error' );
		...
		{
			$message .= '<br /><br />' . sprintf( '<a href="%s">' . __( 'Read more about common connectivity issues.', 'ovic-addon-toolkit' ) . '</a>', 'https://kb.mc4wp.com/solving-connectivity-issues/#utm_source=wp-plugin&utm_medium=mailchimp-for-wp&utm_campaign=settings-notice' );
		...
		public function call( $method, array $data = array() )
		{
			...
			$request_args   = array(
				...
				'sslverify' => apply_filters( 'ovic_use_sslverify', true ),
			);
		...
		private function get_headers()
		{
			...
			$headers = array(
				...
				'User-Agent' => 'ovic/' . OVIC_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' ),
			);
